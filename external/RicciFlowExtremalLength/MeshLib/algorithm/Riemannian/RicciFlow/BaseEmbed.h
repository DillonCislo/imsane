...
{
	for( typename CRicciFlowMesh<CVertex,CEdge,CFace,CHalfEdge>::MeshVertexIterator viter(  m_pMesh ); !viter.end(); viter ++)
  {
    CVertex * v = *viter;
    v->huv() = CPoint2( 0, 0);
    v->touched() = false;
  ...
  for( typename CRicciFlowMesh<CVertex,CEdge,CFace,CHalfEdge>::MeshFaceIterator fiter( m_pMesh ); !fiter.end(); fiter ++ )
  {
    CFace * f = *fiter;
    f->touched() = false;
    root_face = f;
  ...
  // Adds the faces neighboring the root_face to the queue
...
void CBaseEmbed<CVertex,CEdge,CFace,CHalfEdge>::_embed()
{
	...
	while( !m_queue.empty() )
	{
		// Pops the face from the front of the queue - requires that the appropriate neighboring face has already been embedded
		CFace *  head = m_queue.front();
		m_queue.pop();
		assert( head->touched() );

		// Add the surrounding faces to the back of the queue
