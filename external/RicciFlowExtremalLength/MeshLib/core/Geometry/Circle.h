...
inline CCircle orthogonal(  CCircle  C[3] )
{
	...
	//The circle C_i is : <p,p> - 2<p,c_i> = d_i
	double d[3];
	for( int i = 0; i < 3; i ++ )
	{
	...
	CPoint2 centerM = center - C[0].c(); // Adeed by Dillon 2017/08/22

	//double radius = sqrt( mag2(center-C[0].c()) - C[0].r()*C[0].r() );
	// Added by Dillon 2017/08/22
	double radius = sqrt( mag2(centerM) - C[0].r()*C[0].r() );
