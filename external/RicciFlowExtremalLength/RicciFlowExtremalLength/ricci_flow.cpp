        ...
        int nrhs, const mxArray *prhs[])
{
	...
	if (nrhs != 3){
		mexErrMsgIdAndTxt("MATLAB:ricci_flow:nargin",
				"RICCI_FLOW_EXTREMAL_LENGTH requires three input arguments.");
	} else if (nlhs >= 1){
		mexErrMsgIdAndTxt("MATLAB:ricci_flow:nargout",
	...
	for(i=0; i<nrhs; i++){
		if(!mxIsChar(prhs[i]) || mxIsComplex(prhs[i])){
			mexErrMsgIdAndTxt("MATLAB:ricci_flow:inputNotString",
    ...
    if( strcmp(fCommand, "-tangent_ricci") == 0 ) {
        _tangent_ricci(meshFileIn, meshFileOut);
        return;
    }
