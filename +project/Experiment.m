classdef Experiment < handle_light
    ...
    properties (SetAccess = protected)
        ...
        %=========ADDITION BY DILLON CISLO 2/3/2017=====================
%         fitSC         % holds the bounding polygon and map generated by in-plane Schwarz-Christoffel Transformation
        %=========END ADDITION==========================================
        
         %=========ADDITION BY DILLON CISLO 4/11/2017=====================
        allFit  = struct('fitters',[]);       % holds all of the fitter objects for each time points
        %=========END ADDITION==========================================
    ...
    methods
        ...
        function initNew(this)
            ...
            % initialize detectOptions array to default value of
            % detector
...
%                 this.fitter.normallyEvolve(shift, fitOpts.normEvolveSS);
        ...
        %==========ADDITION BY DILLON CISLO 2/2/2017=================
%         function setFitSC(this, varargin)
%             % Sets the fitSC object of a given time point
%             %
%             % setFitSC()
%             % setFitSC(tIdx, SC)
%             % By default sets the SC object corresponding to the current
%             % time to the SC object held in the currently loaded fitter.
%             
%             debugMsg(1, 'Experiment.setFitSC()\n');
%             
%             if nargin == 3
%                 tIdx = varargin{1};
%                 fitSC = varargin{2};
%                 
%                 this.fitSC{tIdx} = fitSC;
%             else
%                 tIdx = this.tIdx(this.currentTime);
%                 fitSC = this.fitter.fitSC;
%                 
%                 this.fitSC{tIdx} = fitSC;
%             end
%             
%         end
        ...
        %==========ADDITION BY DILLON CISLO 4/11/2017=================
        function setAllFit(this, varargin)
            % Sets the allFit object of a given time point
            %
            % setAllFit()
            % setFitSC(tIdx, curFit)
            % By default sets the allFit object corresponding to the current
            % time to the the currently loaded fitter.
            ...
            if nargin == 3
                ...
                this.allFit(tIdx).fitters = curFit;
            else
                tIdx = this.tIdx(this.currentTime);
                curFit = this.fitter;
        ...
        % ----------------------------------------------------
        % Deterimine the ROI manually
        % ----------------------------------------------------
        
        function determineROImanually(this, varargin)
            % use existing fit to determine ROI
            %
            % determineROImanually()
            % determineROImanually(pc)
            %
            ...
            pointCloud = this.detector.pointCloud;
            ManualAxis = varargin{1};
            points = pointCloud.unalignedPoints;
            pc = surfaceDetection.PointCloud(points);
            ManualAxis
            pc.determineROI(5,ManualAxis);
        ...
        % ----------------------------------------------------
        % Deterimine the ROI automatically
        % ----------------------------------------------------
        ...
        function generateSOI(this, varargin)
            ...
            % ============= ADDITION BY DILLON CISLO 09/22/2017 ===========
            popSOIflag = true;
            ...
            for i = 1:length(varargin)
                ...
                if ~isempty( regexp( varargin{i}, '^[Pp]opulate[Ss][Oo][Ii]', 'match') )
                    popSOIflag = varargin{i+1};
                end
            end
            %============= END ADDITION ===================================
            ...
            % populate SOI
            if popSOIflag
                debugMsg(1, 'generateSOI: populating SOI');
                if this.expMeta.dynamicSurface
                    this.fitter.populateSOI(this.SOI, this.currentTime);
                else
